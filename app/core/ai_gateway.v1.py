...
from pyrate_limiter import Duration, RequestRate, Limiter, MemoryListBucket, BucketFullException
...
try:
    from app.config import (
        OPENAI_RPM_PER_KEY,
        OPENAI_RPM_FAIL_FAST,           # bool: if True, raise 429 immediately; else block until token
        OPENAI_MAX_CONCURRENCY_PER_KEY, # int: 0 disables concurrency limiting; else blocks until free
...
def _build_limiter() -> Limiter:
    """
    Build a per-process limiter now; can be switched to Redis-backed later
    by uncommenting the RedisBucket section.
    """
    ...
    # # Optional: switch to Redis (cluster-wide RPM) when ready:
    # if OPENAI_REDIS_URL:
    #     # pip install redis
    #     import redis
...
def call_openai_rate_limited(
    ...
    Behavior:
      ...
      - Applies per-key RPM via pyrate-limiter:
    ...
    try:
        try:
            # delay=True  -> block/pause until we have a token (smooths bursts)
            # delay=False -> raise BucketFullException immediately (fail-fast)
            ...
            # e.meta_info contains:
            #   'reset_in' (seconds till next token),
            #   'remaining' (tokens), etc.
            reset_in = None
            try:
                reset_in = float(e.meta_info.get("reset_in"))  # type: ignore[arg-type]
            except Exception:
                pass
